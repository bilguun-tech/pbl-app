...
data = pd.read_csv("updated_sorted.csv")
df = pd.DataFrame(data)
df = df[["time","time_diff"]]
df.set_index("time", inplace=True)
#print(df.head())


# 成分分解
result=seasonal_decompose(df, model='additive', period=30) # 周期 #加法モデル
...
# オリジナルデータのプロット
plt.subplot(4, 1, 1)
plt.plot(df['time_diff'], label='Original')
plt.legend()
