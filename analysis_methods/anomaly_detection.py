...
def anomaly_detection(original_df, column_name):
    ...
    if xlabel_name == "date":  # 1日ごとの場合
        ...
        df[xlabel_name] = (df[xlabel_name] - pd.Timestamp(xlim_start)) // pd.Timedelta(
            "1s"
        )
    ...
    plt.subplot(2, 1, 1)
    plt.hist(
        scores, bins=50, density=True, alpha=0.5, color="blue", label="Anomaly Scores"
    )
    ...
    if xlabel_name == "date":
        data[xlabel_name] = pd.Timestamp(xlim_start) + pd.to_timedelta(
            data[xlabel_name], unit="s"
        )
    ...
    plt.scatter(
        anomalies[xlabel_name], anomalies[column_name], c="red", label="Anomalies"
    )
